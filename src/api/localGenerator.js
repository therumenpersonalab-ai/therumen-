...
<header class="hero ${heroLayout === "center" ? "center" : ""}"><div class="container hero-grid"><div><div class="badge">${mode} 템플릿</div><h1>${company}<br/>${industry} 맞춤 웹페이지</h1><p>${desc}</p><a class="btn" href="#contact">${ctaText}</a><a class="btn-ghost" href="#structure">구조 보기</a></div>${heroLayout === "split" ? `<img class="hero-img" src="${heroImage}" alt="hero"/>` : ""}</div></header>
